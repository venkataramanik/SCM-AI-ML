# app.py
# TMS Step-by-Step Simulator (with Live Step Panel)
# - Big bold Next Step → (CSS)
# - Live Step Panel shows the current stage output immediately (no tab click needed)
# - Tabs still available for side-by-side drilldown
# - Parcel <=150, LTL <=19,000, else TL/IMDL (auto-intermodal for long-haul if enabled)
# - Hub & Spoke optional
# - TL consolidation (44k lbs), multi-stop, sequential tendering for TL/IMDL only
# - Dashboard for tender & cost analytics
...
def df(rows: List[dict]) -> pd.DataFrame: return pd.DataFrame(rows)
...
seed = st.sidebar.number_input("Random seed", 0, 999999, st.session_state.seed or 11, 1)
...
def parse_lanes(text:str)->Tuple[List[Tuple[str,str]],List[str]]:
    ...
    for tok in tokens:
        ...
        (valid if (o in GEO and d in GEO) else rejected).append(tok if (o not in GEO or d not in GEO) else (o,d))
    # convert valid tuples only
    valid=[v for v in valid if isinstance(v, tuple)]
...
# Step 3: Loads (after rating view)
...
# ── LIVE STEP PANEL (auto-shows current output) ──────────────────
step = int(st.session_state.step or 0)
step_titles = {
    0:"Ready", 1:"Orders Created", 2:"Rating & Mode", 3:"Load Planning", 4:"Tendering", 5:"Shipment Events", 6:"Invoice & Freight Pay"
}
...
if step == 0:
    ...
    } for o in (st.session_state.orders or [])]), use_container_width=True)
elif step == 2:
    ...
    } for ld in (st.session_state.loads or [])]), use_container_width=True)
elif step == 5:
    st.success("Shipment events simulated.")
    st.dataframe(df([asdict(e) for e in (st.session_state.events or [])]), use_container_width=True)
elif step == 6:
    st.success("Invoices generated and matched to plan.")
    st.dataframe(df([asdict(p) for p in (st.session_state.pay or [])]), use_container_width=True)
...
with tabs[7]:
    ...
    if st.session_state.loads is None:
        ...
        loads_df = df([{
            ...
            "stops":" → ".join(getf(ld,"stops",[])),"miles":getf(ld,"miles",0.0),
            "total":getf(ld,"total_est",0.0),"tender":getf(ld,"tender","-"),
            "carrier":(getf(ld,"carrier",None) or "-"),"attempts":getf(ld,"tender_attempts",0)
        } for ld in st.session_state.loads])
        ...
        hub_code_current = hub_code
        hub_usage = loads_df[loads_df["stops"].str.contains(hub_code_current, na=False)]
        st.write(f"Hub Usage ({hub_code_current}) — loads through hub")
