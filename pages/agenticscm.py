...
class Blackboard:
    ...
    trace: List[Dict] = field(default_factory=list)                # full step-by-step logging
...
class ProductionAgent(BaseAgent):
    ...
    def reflect(self, t, env, bb, result):
        made = sum(int(x.split()[1]) for x in result) if result else 0
...
class Orchestrator:
    ...
    def step(self, t: int):
        ...
        self._create_orders(t, self.bb.forecast)  # Create demand based on forecast (sim realism)
        ...
        day_trace["logs"].append(("Planner", sensed["Planner"], plan["Planner"], acted["Planner"], refl))
...
if run:
    ...
    for t in range(1, days+1):
        # stochastic supplier delays (agentic “world events”)
        for po in env.inbound:
            if random.random() > env.suppliers_reliability.get(po.item_id, 0.9):
    ...
    st.info("Set parameters and click **Run Agentic Simulation**.\nThis demo exposes agent loops & policy adaptation (reflection) explicitly.")
